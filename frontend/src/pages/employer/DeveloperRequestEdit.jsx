// src/pages/employer/DeveloperRequestEdit.jsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import EmployerLayout from './components/EmployerLayout';
import { getDeveloperRequestById, updateDeveloperRequest } from '../../services/employerApi';

const DeveloperRequestEdit = () => {
  const { id } = useParams();
  ...
  // Form sabitleri (aynı formdan kopyalandı)
  const PROJECT_TYPES = [
  ...
  const fetchRequest = async () => {
    try {
      ...
      console.error('Talep yüklenirken hata oluştu:', err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateDeveloperRequest(id, formData);
      setSuccess('Yazılımcı talebi başarıyla güncellendi!');
      
      // 2 saniye sonra detay sayfasına yönlendir
      setTimeout(() => {
        navigate(`/employer/developer-requests/${id}`);
      }, 2000);
      
    } catch (error) {
      console.error('Talep güncelleme hatası:', error);
      setError(error.response?.data?.message || 'Talep güncellenirken bir hata oluştu');
    } finally {
  ...
  return (
    <EmployerLayout>
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-gray-800">Yazılımcı Talebini Düzenle</h1>
          <p className="text-gray-600 mt-2">Talep bilgilerini güncelleyebilirsiniz.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 space-y-8">
          ...
          <div className="flex space-x-4 pt-4">
            ...
            <button
              type="button"
              onClick={() => navigate(`/employer/developer-requests/${id}`)}
              disabled={isSubmitting}
...
export default DeveloperRequestEdit;